...
final class TitleBarOverlay: NSView {
    ...
    /// Share-confirmation lives in `ShareConfirmationBanner` (separate
    /// chrome row under the title bar), not in this view — the title
    /// bar never changes mode on Send.
    ...
    /// Containers for the two bar layouts. Both are pinned to the
    /// bar's leading/trailing/centerY edges; `setMode(_:)` flips
    ...
    init() {
        ...
        sendButton.layer?.backgroundColor = ArthurPalette.accent.cgColor
        ...
        idleSendButton.layer?.backgroundColor = ArthurPalette.accent.cgColor
    ...
    /// each container stack. Kept private; the only outside caller is
    /// `setDrawModeActive(_:)`. Dismisses any open picker popover on
    /// transition so it doesn't linger pointing at a now-hidden
    /// trigger button.
